...
export default function AddEntryForm({ user }) {
  ...
  const handleChecked = (e) => {
    ...
    if (e.target.checked === true) {
      const checkedMood = moods.find(
        (mood) => mood.id === parseInt(e.target.id)
      );
  ...
  return (
    <Container>
      <Wrapper>
        <h5>
          Please complete the form with three things you are grateful for today
          and your mood, along with any additional comments :)
        </h5>
